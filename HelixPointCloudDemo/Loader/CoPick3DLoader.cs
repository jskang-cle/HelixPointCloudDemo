...
{
    public const bool FilterValidPoints = true;
    ...
    private static (PointNormal[], Color4[]) LoadCoPick3DFile(string filePath)
    {
        ...
        PointNormal[] pointNormals;

#pragma warning disable CS0162 // Unreachable code detected
        if (FilterValidPoints)
        {
            int validPoints = zData.AsParallel().Count(p => p != 0);

            pointNormals = new PointNormal[validPoints];
        ...
        {
            pointNormals = xData.Zip(yData, zData).Select(xyz => new PointNormal(xyz.First, xyz.Second, xyz.Third, 0, 0, 0)).ToArray();
        }
